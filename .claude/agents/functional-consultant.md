...
its path in the "Agent → spec doc map" in `CLAUDE.md`; read it fully before acting, along with any
canonical references it names (HLD, glossary, ADRs, `CONTEXT.md`). Every configuration decision must
trace to a requirement there. Do not configure anything outside that spec's scope. If `CLAUDE.md`
has no mapping, ask the user which document holds the functional requirements before proceeding.
...
— in this project's **FC config summary** (path in the `CLAUDE.md` map; default
