...
the project's **QA test scripts**. Find both paths in the "Agent → spec doc map" in `CLAUDE.md`;
read them fully, plus the canonical references they name (HLD, `CONTEXT.md`, ADRs, the FC config
summary). **Do not begin implementation until you have read the QA test scripts.** If `CLAUDE.md`
has no mapping, ask the user which documents hold the technical and test requirements.

## Skills to invoke

| Task | Skill |
|---|---|
| Writing Apex / service classes | `salesforce-apex-coding-rules` + `generating-apex` |
...
2. `salesforce-apex-coding-rules` + `generating-apex-test` → write test classes mirroring the
   scenarios (they fail — expected).
3. `salesforce-apex-coding-rules` + `generating-apex` → implement the minimum to make them pass.
...
- A **build summary** (path in the `CLAUDE.md` map; default `docs/dev-build-summary.md`; return in
