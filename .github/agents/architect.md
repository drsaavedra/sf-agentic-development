...
it names (HLD, `CONTEXT.md`, the ADRs) and the other agents' spec + summary docs. If `AGENTS.md`
exists at the repo root, find the paths there. Otherwise, ask the user which documents hold the
architecture and requirements before proceeding. Every finding must cite a specific requirement
(spec/HLD/ADR section).
...
Append findings, per gate, to this project's **review report** (path in `AGENTS.md` if defined; default
`docs/sa-review-report.md`; return in chat if neither exists), under a clearly labelled `## Gate 1` or `## Gate 2` section:
