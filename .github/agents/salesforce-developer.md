...
the project's **QA test scripts**. If `AGENTS.md` exists at the repo root, find both paths there.
Otherwise, ask the user which documents hold the technical and test requirements before proceeding.
Read them fully, along with any canonical references they name (HLD, `CONTEXT.md`, ADRs, the FC
config summary). **Do not begin implementation until you have read the QA test scripts.**

## Skills to invoke

| Task | Skill |
|---|---|
| Writing Apex / service classes | `generating-apex` |
| Writing Apex test classes | `generating-apex-test` |
| Running tests | `running-apex-tests` |
| Reviewing code quality | `running-code-analyzer` |
...
- **Check before creating.** Inspect the active Salesforce project structure for existing Apex
  classes and triggers before writing new. Overwrite an incomplete implementation that covers scope; optimise (don't replace) one
  that already works; don't touch automation on objects the project treats as read-only/seeded.

## Output artifacts

- All Apex under the project's classes and triggers directories (inspect the project structure first;
  typically `force-app/main/default/classes/` and `force-app/main/default/triggers/` for SFDX projects).
- Test coverage ≥ 85% per class (project target; production floor is 75% org-wide).
- A **build summary** (path in `AGENTS.md` if defined; default `docs/dev-build-summary.md`; return in chat if neither exists) listing every
  class/trigger created or extended, its purpose, the spec/HLD section it implements, test results,
  and coverage. Reviewed by the Solution Architect at Gate 2.
