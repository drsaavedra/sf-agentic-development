...
- **API version sets the security baseline — check it first.** As of API v67 (Summer '26): database operations (SOQL, SOSL, DML, `Database` methods) **default to user mode**; a class with **no sharing declaration defaults to `with sharing`** (previously implicit `without sharing`); and **triggers always run in system mode** and can no longer declare sharing or access modes. For classes below v67, none of these defaults apply — explicit `WITH USER_MODE` and an explicit sharing keyword are required. Explicit declarations remain preferred at every API version: they survive API bumps and state intent.
...
- **`WITH SECURITY_ENFORCED` is removed at API v67 — it no longer compiles.** In pre-v67 code treat it as deprecated and migrate on sight to `WITH USER_MODE`, which also handles polymorphic fields, checks the whole query (not just SELECT/FROM), and reports every access error instead of aborting on the first.
